...
class Functions extends Database
{
  ...
  **/
  ...
  {
    $sql = "SELECT * FROM {$this->tableName} WHERE catid={$id} AND isactive=1 LIMIT {$start},{$limit}";
  ...
  public function getCount($id)
    {
        $sql = "SELECT count(*) as pcount FROM {$this->tableName} WHERE catid={$id} AND isactive=1";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
